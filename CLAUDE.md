...
python knowledge_graph.py <path-to-graph.json> --cytoscape output.html --compound-groups
...
- **Compound document groups:** `cytoscape_elements(compound_groups=True)` uses Cytoscape.js compound (parent) nodes to represent document context. Document and section nodes become transparent bounding-box containers; entity nodes are nested inside their source section via the `parent` field. All structural edges (`documented_by`, `part_of`, `contains`, `documents`) are removed so only knowledge-bearing relations remain. Stats, type filters, and relation filters reflect only content nodes. Available via CLI (`--cytoscape output.html --compound-groups`), web UI ("Grouped" toggle button on graph detail page, `?compound=true` query parameter), and programmatic API (`kg.cytoscape_elements(compound_groups=True)`).
